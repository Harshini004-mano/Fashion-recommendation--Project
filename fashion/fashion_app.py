...
# --------------------------
#  Replace / keep your full default_rules dictionary here
#  (Use the big dictionary you already have with "tip" and "sample_outfits")
# --------------------------
default_rules = {
    "Women": {
        "Hot": {
...
st.write("Select category, temperature and place to see tips, sample outfits and images.")

# Dropdowns
category = st.selectbox("Category", ["men", "women", "children_boy", "children_girl"])
temperature = st.selectbox("Temperature", ["hot", "cold", "rainy", "sunny", "overcast"])
place = st.selectbox("Place", ["beach", "park", "restaurant", "city", "mountains"])

# Build folder path for images
folder_path = f"images/{category}/{temperature}/{place}"

# Map category dropdown to dictionary keys
if category == "children_boy":
    ...
    dict_top = category.capitalize()  # "men" -> "Men", "women" -> "Women"
    dict_sub = None

# Lookup the rule
rule = None
temp_key = temperature.capitalize()  # "cold" -> "Cold"
try:
    if dict_sub:
...
# Display tip and sample outfits
if rule:
...
if os.path.exists(folder_path):
    ...
    st.warning("Image folder for this combination does not exist.")
