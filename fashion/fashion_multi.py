...
# --- PAGE 2: CATEGORY ---
    ...
    st.session_state.category = st.selectbox("Select Category", ["Men", "Women", "Children"])
    if st.session_state.category == "Children":
        st.session_state.child_type = st.selectbox("Select Type", ["Boy", "Girl"])
    st.button("⬅️ Back", on_click=prev_page)
    st.button("➡️ Next", on_click=next_page)

# --- PAGE 3: TEMPERATURE ---
elif st.session_state.page == 3:
    st.header("🌤️ Step 2: Select Temperature")
    st.session_state.temperature = st.selectbox(
        "Select Temperature", ["Hot", "Cold", "Rainy", "Sunny", "Overcast"]
    )
    st.button("⬅️ Back", on_click=prev_page)
    st.button("➡️ Next", on_click=next_page)
...
# --- PAGE 4: PLACE ---
    ...
    st.session_state.place = st.selectbox(
        "Select Place", ["Beach", "Park", "Restaurant", "City", "Mountains"]
    )
    st.button("⬅️ Back", on_click=prev_page)
    st.button("➡️ Next", on_click=next_page)
...
# --- PAGE 5: RECOMMENDATION ---
    ...
    try:
        ...
        st.warning("⚠️ This combination does not exist yet!")
    ...
    if os.path.exists(image_folder):
        ...
        if image_files:
            ...
            st.warning("⚠️ No images found in this category.")
    else:
        st.warning("⚠️ Image folder not found! Please check your folder structure.")
